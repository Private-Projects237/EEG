...
% nfft_factor = multiplier to added zeroes (recommend 1)
% fig: ("Yes"/"No")
% xmax: Max fz in plot
%
% Example:
...
function [welchx_info] = welchx(signal, srate, winsec, nOverlap_per, nfft_factor, fig, xmax)
    ...
    nfft = 2^nextpow2(winlength) * nfft_factor; % Number of FFT time points
    ...
    for wi = 1:length(winonsets)
        ...
        AmplitudeW = 2 * abs(fft(datachunkH,nfft)) / hannNorm;
