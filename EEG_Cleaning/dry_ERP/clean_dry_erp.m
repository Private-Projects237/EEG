...
% markerNames: The names of the markers in the EEG data (must be a cell array)
...
function clean_dry_rseeg(eegFiles, EEG_Pathway, EEG_save_path, EEG_csv_save_path, preprocParams, batchSize)
    ...
    for startIdx = 1:batchSize:numFiles
        ...
        parfor i = 1:length(currentBatch)
            ...
            result = clean_dry_erp_result_struct(currentBatch{i});
            ...
            try
                ...
                % === Frequency of Markers Before any EEG removal
                result.InitialRecordingLenMin = round(size(EEG.data,2) / (EEG.srate * 60),2);
                result.InitialMarkers = uniqueMarkerCount(EEG);

                % === Keep EEG segments with marker info only ===
                EEG = keep_marker_EEG_segments(EEG,...
                    preprocParams.markerNames,...
                    preprocParams.markerLatThresh,...
                    preprocParams.baselineLat,...
                    preprocParams.postStiLat);

                % === Frequency of Markers After non Marker EEG removal
                result.RemainingRecordingLenMin = round(size(EEG.data,2) / (EEG.srate * 60),2);
                result.RemainingMarkers = uniqueMarkerCount(EEG);
